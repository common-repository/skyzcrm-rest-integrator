...
function skyz_plc_rest_post($plc_token, $category, $data,$sendtoonfail,$siteurl) {
    $json = '';
    ...
    if (!is_wp_error($response)) {
		skyz_db_update_lead($localrecid,'');
        $json = json_decode($response['body'], true);
    }else{
		  //$response_message = wp_remote_retrieve_response_code( $response );
		  $response_message = $response->get_error_message();
		  skyz_db_update_lead($localrecid,$response_message);
		  if(isset($sendtoonfail)){
          $subject = 'Skyz connectivity error';
          $message = "Site ".$siteurl."
          Report local objectid=".$localrecid." can't be automaticaly sent to skyz
          Reason:
          ".$response_message."
          Best regards SkyzCrm Rest integrator plugin.";
          wp_mail( $sendtoonfail, $subject, $message );
          }
		}
...
function skyz_SingleField($fname, $fval) {
	$ret='';
	if (isset($fval)){
	$ret = json_encode(array($fname => $fval));
...
function skyz_Lead($name, $notes, $phone, $email, $ownerid, $campaignid) {
    ...
    if (isset($ownerid) && $ownerid !== '') {
        $ownerob = '"cat_7186":[{"field_7187":"'.$ownerid.'","recid":"1004","entityTypeId":"2111102000000" ,"isLink":true } ],';
    }
    $campob = '';
    if (isset($campaignid) && $campaignid !== '') {
        $campob = '"cat_7149":[{"field_7143":"plc_const_link_stage_first","field_7150":"'.$campaignid.'" ,"recid":"1005","entityTypeId":"2111103000000","isLink":true}],';
    }
    $notes = skyz_SingleField('field_4052' , $notes);
    $name = skyz_SingleField('field_1016', $name);
    
    return '{' . $name . ',' . $notes . ',"cat_7087":[' . $phob . ($phob !== '' ? ',' : '') . $mailob . '],' . $campob . '' . $ownerob . '"recid":"1001","entityTypeId":"2111403020000" ,"isLink":false}';
}
...
function skyz_sendrestform($contact_form) {
    ...
    if ($submission) {
        ...
        //wpcf7-mail-sender,wpcf7-mail-recipient
        $mailProp = $contact_form->get_properties('mail');
        $sendtoonfail=$mailProp['mail']['recipient'];
        ...
        if (isset($onstage) && isset($posted_data[$onstage]) && $posted_data[$onstage] !== '') {
            ...
            if ($category !== '') {
                ...
                skyz_plc_rest_post($token, $category, $data,$sendtoonfail,$url);
            }
        }
    }
}
